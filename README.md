# CVF Citation Analyzer

CVPR / ICCV / WACV 採択論文の引用数を Semantic Scholar API で取得し，リンク種別・発表種別ごとに比較・可視化するツール．
...
├── conf_utils.py             # 会議別ディレクトリ・設定の共通ユーティリティ
│
├── retry_normalized.py       # タイトル正規化・再検索・conf_match フラグ付与
├── export_corrections.py     # conf_match=False の要確認論文を JSON に書き出す
├── apply_corrections.py      # 手動記入済み JSON を _fixed キャッシュに反映する
│
├── scrape_links.py           # リンク種別・発表種別の収集（CVPR/ICCV/WACV 対応）
...
会議ごとにサブディレクトリが分かれる（例: ICCV → `output/cache_iccv/`, `output/result_iccv/`）．
...
| 会議 | 対応年度 | キャッシュディレクトリ |
|---|---|---|
| CVPR | 2023, 2024, 2025 | `output/cache/`, `output/result/` |
| ICCV | 2021, 2023 | `output/cache_iccv/`, `output/result_iccv/` |
| WACV | 2022, 2023, 2024, 2025 | `output/cache_wacv/`, `output/result_wacv/` |
...
全体は 3 段階に分かれる．会議・年度ごとに繰り返す．

```
[自動] bash run_full.sh
           ↓ output/cache{CONF_SUFFIX}/corrections_{YEAR}.json を生成
[手動] corrections_{YEAR}.json の "url" 欄を記入
           ↓
[自動] bash run_corrections.sh
           ↓ output/cache{CONF_SUFFIX}/cache_citations_{YEAR}_fixed.json を更新
[自動] bash run_statistics.sh
           ↓ 統計・グラフを output/result{CONF_SUFFIX}/ に保存
```
...
結果は `output/cache{CONF_SUFFIX}/cache_citations_{YEAR}.json` にキャッシュされるため，Ctrl+C で中断しても再実行時は途中から再開できる．

#### [2] retry_normalized.py — タイトル正規化・再検索・conf_match 付与

タイトルに表記揺れ（二重スペース・`^N` 記法・カーリークォート等）がある論文を正規化クエリで再検索し，マッチ精度を改善する．  
また全エントリに `cvpr_match` フラグを付与する（キー名は後続スクリプトとの互換性で維持）：

| `cvpr_match` | 条件 |
|---|---|
| `True` | `venue` に各会議名を含む かつ `year ∈ {YEAR-2, YEAR-1, YEAR}` |
| `False` | それ以外（別会議・別年度・arXiv のみ など） |

結果は `output/cache{CONF_SUFFIX}/cache_citations_{YEAR}_fixed.json` に保存される．

#### [3] export_corrections.py — 修正候補の書き出し

`cvpr_match=False` のうち，`similarity < 1.0` かつ `paper_year >= YEAR-2` のエントリを  
`output/cache{CONF_SUFFIX}/corrections_{YEAR}.json` に書き出す．

---

## Stage 2 — 手動修正（`corrections_{YEAR}.json` の編集）

`output/cache{CONF_SUFFIX}/corrections_{YEAR}.json` を開き，各エントリの `"url"` 欄を記入する．
...
`url` 欄に URL が指定されたエントリは Semantic Scholar API で正しいデータを再取得してから保存する．
...
CVF の採択論文ページから全採択論文のリンク情報を収集し，`output/result{CONF_SUFFIX}/{conf}{YEAR}_links.csv` に保存する．HTML は `output/cache{CONF_SUFFIX}/` にキャッシュされるため，2 回目以降はネットワークアクセスなしで動作する．
...
| `link` | リンク URL（なければ空） |
| `presentation_type` | `highlight` / `award_candidate`（CVPR 2023 のみ） / `oral`（ICCV） / `poster` |
...
| `comparison_{YEAR}_03_presentation.png` | 発表種別 | Highlight / Poster（/ Award Candidate / Oral） |
| `comparison_{YEAR}_04_ptype_x_link.png` | 発表種別 × リンク有無 | （各組み合わせ） |
| `comparison_{YEAR}_05_link_type_4way.png` | リンク種別（4 値） | GitHub / GitHub.io / Project Page / No Link |
| `comparison_{YEAR}_06_link_type_3way_gh.png` | リンク種別（3 値，GitHub.io を Project Page に統合） | GitHub / Project Page / No Link |
| `comparison_{YEAR}_07_link_type_4way_log.png` | リンク種別（4 値，対数変換） | GitHub / GitHub.io / Project Page / No Link |

統計量（n / mean / median / std / Q25 / Q75 / max）は `citation_comparison_{YEAR}.csv` / `.json` にも保存される．

#### [3] topn.py — 全年度 Top-N 引用論文の抽出

`output/cache{CONF_SUFFIX}/cache_citations_202*_fixed.json` を全年度分読み込み，引用数上位 TOP_N 件を年度別・全年度まとめの両形式で出力する．

| ファイル | 内容 |
|---|---|
| `output/result{CONF_SUFFIX}/output_{year}/top{N}.csv` | 年度別ランキング |
| `output/result{CONF_SUFFIX}/top{N}_all_years.csv / .json` | 全年度まとめ |
...
| ファイル | 内容 |
|---|---|
| `output/result{CONF_SUFFIX}/similarity_summary.csv` | 年度別サマリー |
| `output/result{CONF_SUFFIX}/similarity_details.csv / .json` | 類似度 < 1.0 の論文一覧 |
...
CVPR の場合（`CONF_SUFFIX` = 空），ICCV は `_iccv`，WACV は `_wacv` が付く．
...
│   ├── cache_citations_{year}_fixed.json
│   ├── corrections_{year}.json
│   └── cache_cvpr{year}_accepted.html
├── cache_iccv/                         # ICCV キャッシュ（同構造）
├── cache_wacv/                         # WACV キャッシュ（同構造）
│
├── result/                             # CVPR 分析結果
│   ├── output_{year}/
...
│   ├── comparison_{year}_0N_*.png
│   ├── top{N}_all_years.csv / .json
│   ├── similarity_summary.csv
│   └── similarity_details.csv / .json
├── result_iccv/                        # ICCV 分析結果（同構造）
└── result_wacv/                        # WACV 分析結果（同構造）
```
